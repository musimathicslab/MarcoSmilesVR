...
def save_stats(statistiche, titolo_tabella=""):
    ...
    plt.title(f"Stats for {titolo_tabella}", fontsize=14, pad=20)
    plt.tight_layout()
    plt.savefig(output_image, dpi=300)
    plt.close()
    print(f"Tabella salvata come: {output_image}")

def extract_batch_number(suffix):
    """Estrae il numero dal suffisso tipo 'batch64'."""
    match = re.search(r'\d+', suffix)
    return int(match.group()) if match else float('inf')

# === ORDINA CSV PER BATCH SIZE ===
...
def Plot(keyword):
    ...
    csv_files.sort(key=lambda f: extract_batch_number(split_name(os.path.splitext(os.path.basename(f))[0])[1]))
    prefix_comune = None
    ...
    ax2.grid(True)
    ax2.legend()
    fig.suptitle(f"Results for {prefix_comune}", fontsize=14)

    plt.tight_layout(rect=[0, 0.03, 1, 0.95])

    plt.savefig(prefix_comune,dpi=300,bbox_inches='tight')
    plt.show()
