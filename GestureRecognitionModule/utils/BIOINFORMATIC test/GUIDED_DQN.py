...
class DQNAgent:
    ...
    def get_action(self, state, eps=0.20, forced_action=None):
        ...
        if random_value < eps and real_action_prob != 2:
            return self.env.action_space.sample()
        elif random_value < real_action_prob:
    ...
    def compute_loss(self, batch):
        ...
        curr_Q = self.model(states).gather(1, actions.view(-1, 1))

        # Compute next Q values from target model
...
def save_training_log_csv(df_log: pd.DataFrame, train_dataset_path: str, batchsize: int,addinfo):
    ...
    csv_filename = f"{additional_info}_{script_name}_{train_dataset_name}_batch{batchsize}.csv"
...
def validate(epoch,elapsed_time,dataset_test,agent):
    ...
    print(f'test Accuracy on epoc {epoch}: {accuracy}')
...
def train(batch,epochs,guided):
    ...
    for e in range(epochs):
        ...
        for episode in range(episodes):
            ...
            for step in range(max_steps):
                ...
                state = next_state
            #print("DONE")
            #print(f"EPOCHS {e+1}/{epochs}   Episode: {episode + 1}, Total Reward: {episode_reward}")
    ...
    print(df_log)
    save_training_log_csv(df_log,dataset_test_path,batch,addinfo)
    ...
    model_name="BIO_e"+str(epocs)+"_"+todaydate.strftime(date_format)

    model_name="BIO"+str(epocs)+"ep_DQN"
